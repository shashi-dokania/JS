...
for(var i = 0; i < ingredientsArray.length; i++){
  funcInvoker(breadMaker,'italian');
}
...
//telegraphPrepLearning(jsPoints);
...
var call5SecondsLater = function(callback) {
  ...
  var timeCall = setTimeout(callback, 5000);
  return timeCall;
};
...
// 7. Try creating various functions and passing them in as the argument for call5SecondsLater. 
  ...
  var greeting = function() {
    console.log('Hello ' + 'Clarissa');
  }
  call5SecondsLater(greeting);
...
var signUpNewUser = function() {
  var msg = 'Confirmed: your new account is created!';
  console.log (msg);
};

var signUpAgain = function(){
  console.log('Whoops, it seems you already have an account! Did you mean to sign in?');
};
...
multiplyOnce(3,5); // 15
multiplyOnce(4,8); // nothing should happen here. 
