...
// 1. Add 5 to each item in testArr
  ...
  var newArray = _.map(testArr, function(num){return (num + 5);});
  console.log(testArr);
...
// 2. Let's do another transform. 
  ...
  var cityNameArray = _.map(shortNameArr, function(city){return cityMapper[city];});
...
// 3. Now let's use map to take an array of strings, and return an array of objects. 
  ...
  var colorObj = _.map(colorArray,function(val){return {color: val};});
...
_.each(arr, function(){console.log('hi, I\'m inside the callback!');});
...
// 2. Cool, now let's console.log what item, the first parameter for our callback, is for each iteration.
  ...
  // Remember that a callback function is just a block of code that we execute the same number of times as we have items in the array. 
    ...
    _.each(arr, function(element){console.log(element);});
...
// 3. Declare a variable called count that exists in the global scope, and set it equal to 0. Now that you're comfortable accessing each item in arr, let's console log both count (which will represent the index of that item) and the relevant value stored in arr. So with each iteration we'll want to increase count by 1. 
  ...
  var count = 0;
  _.each(arr, function(element){console.log('count is: ' + count +', the value we\'re accessing in the array is ' + element);count++;});
...
var addOne = [];
_.each(arr, function(element){element += 1; addOne.push(element);});
console.log(addOne);

var multiplyByFive = [];
_.each(arr, function(element){element *= 5; multiplyByFive.push(element);});
console.log(multiplyByFive);

var divideByTen = [];
_.each(arr, function(element) {element /= 10; divideByTen.push(element);});
...
_.each(obj, function(){console.log('hi, i\'m inside the callback!')});
...
var arrayOfKeys = [];
_.each(obj, function(val,key){arrayOfKeys.push(key);});
...
// 7. Ok, now that we've figured out that key is just a variable that will be set equal to the name of each property in obj, 
  ...
  // 10
var arrayOfValues = [];
_.each(obj, function(val,key){arrayOfValues.push(val);});
...
_.each(obj, function(val,key){console.log('the value of ' + key + ' is: ' + val);});
...
colorArray = ['red','purple','orange','yellow','green','purple'];
var findPurpleInArray = _.filter(colorArray, function(val) {return val === 'purple';});
console.log(findPurpleInArray);
//console.log(colorObj);
var findPurpleInObj = _.filter(colorObj, function(val){ return val.color === 'purple'; });
console.log(findPurpleInObj);
